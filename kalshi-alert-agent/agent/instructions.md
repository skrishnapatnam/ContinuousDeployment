...
Find open Kalshi sides that offer **≥2× profit** (ask ≤ ~$0.33) **and** whose **series settlement history** implies a **≥99% chance** that side wins (from past expiration values / comparable strikes).
...
3. Enforce both floors: **profit ≥ 2× cost** and **historical win rate ≥ 99%** (enough samples).
4. Prefer historical win rate in the headline, then ask, profit multiple, sample size, ticker, URL.
